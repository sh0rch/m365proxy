"""Proxy handlers for SMTP and POP3 protocols."""
# -----------------------------------------------------------------------------
...
def start_pop3(host: str, port: int) -> ThreadedTCPServer:
    """Start a threaded TCP server for POP3 protocol."""
    ...
    def handle(self):
        ...
        try:
            while True:
                ...
                if not line:
                    break
                logging.debug(f" {line if line[:5].upper() not in [
                    'PASS ',
                    'PLAIN'
                ] else f'{line[:5].strip()} *****'}")
                ...
                if cmd == "USER":
                    ...
                    if not self.username or not args or not check_credentials(
                            self.username, args[0]):
                        self.wfile.write(b'-ERR invalid credentials\r\n')
                        return
                    try:
                        self.messages = loop.run_until_complete(
                            list_messages(self.username))
                        self.uids = [msg["id"] for msg in self.messages]
                        self.deleted.clear()
                        self.cache.clear()
                        logging.info(
                            f"Retrieved {len(self.messages)} messages "
                            f"for {self.username}")
                    except Exception:
                    ...
                    self.wfile.write(
                        f"+OK {len(self.messages)} messages:\r\n".encode())
                    ...
                    try:
                        idx = int(args[0]) - 1
                        if idx < 0 or idx >= len(self.messages) or \
                                idx in self.deleted:
                            self.wfile.write(b'-ERR no such message\r\n')
                            continue
                        ...
                        if msg_id in self.cache:
                            ...
                            raw = loop.run_until_complete(
                                get_message_raw(self.username, msg_id))
                        ...
                        self.wfile.write(
                            b'-ERR failed to retrieve message\r\n')
                    ...
                    if self.deleted:
                        ...
                        for idx in self.deleted:
                            try:
                                ...
                                logging.info(f"Deleted message {msg_id} for {
                                    self.username}")
                            except Exception:
                                logging.error(f"Failed to delete message {
                                    self.messages[idx]['id']}")
